...
export class PlayersMgmtComponent implements OnInit {
  ...
  addPlayer(player: any, userId: number, imagePlayer: string | File) {
    this.accessService.addPlayer(player, userId).subscribe({
      next: (result: any) => {
        ...
        //{ success: true, message: 'OK', data: { id: 48 } }
  ...
  /*   updatePlayerData(userId: number, player: any, imagePlayer: File) {
      this.accessService.updatePlayerData(userId, player).subscribe({
        next: (result: any) => {
          ...
          //this.showSnackBar('El jugador se ha agregado');
          setTimeout(() => {
            this.getPlayerList();
          }, 1000);

          //this.getPlayerList();
          this.showSnackBar('El jugador se ha actualizado');
        },
        error: (error: any) => {
          console.log(error);
          this.showSnackBar('El jugador no se ha actualizado');
        }
      });
    } */
        ...
        next: (result: any) => {
          ...
          if (result.success) {
            ...
            if (typeof imagePlayer === 'string') {
              ...
              const fileName = `image_${formattedDate}.jpg`;
